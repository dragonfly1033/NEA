...
class Button:
    def __init__(
        ...
        self.rect = list(rect)
        self.labelRect = [
            self.rect[0] + (borderWidth * 2),
            self.rect[1] + (borderWidth * 2),
            self.rect[2] - (borderWidth * 4),
            self.rect[3] - (borderWidth * 4),
        ]
        self.text = text
        self.fg = fg
    ...
    def show(self):
        ...
        self.button_text1 = Label(
            display,
            self.text,
            self.labelRect,
            self.font,
            self.colours[self.active],
            self.fg,
            align="centre",
            justify="centre",
            addSelf=False,
        ...
        pg.draw.rect(
            ...
            self.colours[self.active],
            (
                self.rect[0] + self.borderWidth,
                self.rect[1] + self.borderWidth,
                self.rect[2] - (2 * self.borderWidth),
                self.rect[3] - (2 * self.borderWidth),
            ),
        )
    ...
    def update(self, event):
        ret = False
        updatedBefore = self.display.getScreen().totalUpdates.count(
            True
...
class Input:
    ...
    def isWider(self):
        return (
            self.getTextWidth()
            > self.rect[2] - (2 * self.borderWidth) - self.textOffset
        )

    def show(self):
        c = self.fg if self.activated > 0 else [min(255, (i + 75)) for i in self.fg]
        self.textR = self.font.render(
            self.text[self.homeCursorPos : self.endCursorPos + 1], True, c
        )
        if isinstance(self.display, ScrollableSurface):
            display = self.display.contentSurface
        else:
    ...
    def show_cursor(self):
        if self.active and sin(pg.time.get_ticks() * pi / self.cursorSpeed) > 0:
            ...
            textWidth = (
                self.font.size(self.text[self.homeCursorPos : self.cursorPos])[0]
                + self.borderWidth
                + 5
            )
            pg.draw.line(
                ...
                self.fg,
                (
                    self.rect[0] + textWidth,
                    self.rect[3] / 2 - lineHeight / 2 + self.rect[1],
                ),
                (
                    self.rect[0] + textWidth,
                    self.rect[3] / 2 + lineHeight / 2 + self.rect[1],
        ...
        if event.type == pg.KEYDOWN:
            if self.active:
                if event.key == pg.K_BACKSPACE:
                    ...
                    if self.cursorPos == self.endCursorPos and self.endCursorPos < len(
                        self.text
                    ):
                        self.endCursorPos += 1
                    while self.isWider():
                ...
                if self.cursorPos < 0:
                    self.cursorPos = 0
                if self.homeCursorPos < 0:
                    self.homeCursorPos = 0
                if self.endCursorPos < 0:
                    self.endCursorPos = 0
                if self.cursorPos > len(self.text):
                    self.cursorPos = len(self.text)
                if self.homeCursorPos > len(self.text):
                    self.homeCursorPos = len(self.text)
                if self.endCursorPos > len(self.text):
                    self.endCursorPos = len(self.text)
    ...
    def update(self):
        ...
        if self.align == "right":
            ...
            self.label_rect.left = (
                self.rect[0] + ((self.rect[2] / 2) - (self.label_rect.width / 2)) // 1
            )
        ...
        if self.justify == "top":
            ...
            self.label_rect.top = (
                self.rect[1] + ((self.rect[3] / 2) - (self.label_rect.height / 2)) // 1
            )
...
class DropDown:
    def __init__(
        ...
        self.zlayer = zlayer
        self.down = False
        self.contentSurface = pg.Surface(
            (
                self.rect[2] - self.borderWidth * 2,
                self.dropHeight - self.borderWidth * 2,
            )
        )
        self.layer = layer
        self.widgets = []
        ...
        self.button_rect = (
            self.rect[0] + self.rect[2] * 0.8 - self.borderWidth,
            self.rect[1],
            self.rect[2] * 0.2 + self.borderWidth,
            self.rect[3],
        )
        self.label = Label(
            self.display,
    ...
    def show(self):
        if self.down:
            pg.draw.rect(
                ...
                (
                    self.rect[0],
                    self.rect[1] + self.rect[3] - self.borderWidth,
                    self.rect[2],
                    self.dropHeight,
                ),
            )
            ...
            self.layer.blit(
                self.contentSurface,
                (self.rect[0] + self.borderWidth, self.rect[1] + self.rect[3]),
            )
...
class ScrollableSurface(pg.Surface):
    ...
    def addWidget(self, other):
        self.widgets.append(other)
        self.contentSurface = pg.transform.scale(
            self.contentSurface, (self.contentRect[2], self.contentRect[3])
        )
        self.excess = [
            self.contentRect[2] + self.padding - self.showRect[2],
            self.contentRect[3] + self.padding - self.showRect[3],
        ]
    ...
    def isOver(self):
        ...
        if isinstance(self.display, DropDown):
            x, y = (
                x - self.display.rect[0] - self.display.borderWidth,
                y - self.display.rect[1] - self.display.rect[3],
            )
    ...
    def update(self, event):
        ...
        if event.type == pg.MOUSEMOTION:
            if self.barGrab:
                ...
                self.ratio = 1 - (y - (2 * self.padding)) / (
                    self.showRect[3] - (2 * self.padding) - self.barHeight
                )
                self.ratio = min(max(self.ratio, 0), 1)
                self.show()
    ...
    def show(self):
        ...
        self.barStart = (1 - self.ratio) * (
            self.showRect[3] - (2 * self.padding) - self.barHeight
        ) + self.padding
        self.blit(self.contentSurface, (self.offset[0], self.offset[1]))
        self.bar = pg.draw.rect(
            self,
            self.colours[self.barGrab],
            (barx, self.barStart, self.barw, self.barHeight),
        )
        if isinstance(self.display, Screen):
            display = self.display
        else:
...
class Screen(pg.Surface):
    ...
    def event_update(self, event):
        ...
        for layer in embedKeys:
            if layer not in self.layeredUpdates:
                self.layeredUpdates[layer] = []
